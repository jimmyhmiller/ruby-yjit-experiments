...
fn test_base_insn_out() {
    ...
    asm.mov(Opnd::mem(64, SP, 8), Opnd::mem(64, SP, 0));
...
fn test_jcc_ptr() {
    ...
    let side_exit =
        Target::CodePtr(((cb.get_write_ptr().raw_ptr() as usize + 4) as *mut u8).into());
    let not_mask = asm.not(Opnd::mem(32, EC, RUBY_OFFSET_EC_INTERRUPT_MASK));
    asm.test(Opnd::mem(32, EC, RUBY_OFFSET_EC_INTERRUPT_FLAG), not_mask);
    asm.jnz(side_exit);
...
fn test_cmp_8_bit() {
    ...
    asm.cmp(
        Opnd::Reg(reg).with_num_bits(8).unwrap(),
