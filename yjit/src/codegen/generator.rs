...
use crate::{
    ...
    cruby::{
        ...
        get_cme_def_body_optimized_type, get_cme_def_type, get_def_iseq_ptr, get_def_original_id,
        get_ec_cfp, get_iseq_body_local_iseq, get_iseq_body_local_table_size,
        ...
        get_iseq_flags_has_post, get_iseq_flags_has_rest, get_iseq_flags_ruby2_keywords,
        get_mct_argc, get_mct_func, imemo_callinfo, insn_len, iseq_inline_constant_cache,
        ...
        rb_hash_stlike_foreach, rb_hash_stlike_lookup, rb_iseq_pc_at_idx, rb_iseq_t, rb_ivar_get,
        rb_leaf_builtin_function, rb_mRubyVMFrozenCore, rb_obj_as_string_result, rb_obj_is_kind_of,
        ...
        rb_shape_id_offset, rb_shape_transition_shape_capa, rb_str_concat_literals,
        ...
        ruby_basic_operators, st_data_t, vm_ci_argc, vm_ci_kwarg, vm_ci_mid, EcPtr, IseqPtr,
        Qfalse, Qnil, Qtrue, Qundef, RBasic, RedefinitionFlag, YARVINSN_putobject_INT2FIX_0_,
        ...
        RUBY_OFFSET_RBASIC_KLASS, RUBY_OFFSET_RSTRUCT_AS_ARY, RUBY_OFFSET_RSTRUCT_AS_HEAP_PTR,
        RUBY_SIZEOF_CONTROL_FRAME, RUBY_SPECIAL_SHIFT, RUBY_SYMBOL_FLAG, RUBY_T_ARRAY, RUBY_T_HASH,
        RUBY_T_ICLASS, RUBY_T_MASK, RUBY_T_MODULE, RUBY_T_OBJECT, RUBY_T_STRING, RUBY_T_STRUCT,
        SHAPE_ID_NUM_BITS, SIZEOF_VALUE, SIZEOF_VALUE_I32, STRING_REDEFINED_OP_FLAG, ST_CONTINUE,
        ST_STOP, VALUE, VALUE_BITS, VM_BLOCK_HANDLER_NONE, VM_CALL_FCALL, VM_CALL_KW_SPLAT,
        VM_CALL_OPT_SEND, VM_ENV_DATA_INDEX_FLAGS, VM_ENV_DATA_INDEX_ME_CREF,
        VM_ENV_DATA_INDEX_SPECVAL, VM_ENV_DATA_SIZE, VM_ENV_FLAG_LOCAL, VM_ENV_FLAG_WB_REQUIRED,
        VM_FRAME_FLAG_BMETHOD, VM_FRAME_FLAG_CFRAME, VM_FRAME_FLAG_CFRAME_KW, VM_FRAME_FLAG_LAMBDA,
...
impl CodeGenerator {
    ...
    pub fn jump_to_next_insn(&mut self) {
        ...
        if self.jit.record_boundary_patch_point {
            ...
            let exit_pos = self.gen_outlined_exit(exit_pc, &reset_depth);
    ...
    pub fn jit_ensure_block_entry_exit(&mut self) {
        ...
        if self.jit.insn_idx == blockid.idx {
            ...
            block.entry_exit = Some(self.gen_outlined_exit(block_entry_pc, &block_ctx));
        }
    }

    // Generate RARRAY_LEN. For array_opnd, use Opnd::Reg to reduce memory access,
    // and use Opnd::Mem to save registers.
    pub fn get_array_len(&mut self, array_opnd: Opnd) -> Opnd {
    ...
    pub fn gen_nop(&mut self) -> CodegenStatus {
        // Do nothing
        CodegenStatus::KeepCompiling
    }

    pub fn gen_pop(&mut self) -> CodegenStatus {
        // Decrement SP
        self.ctx.stack_pop(1);
        CodegenStatus::KeepCompiling
    }

    pub fn gen_dup(&mut self) -> CodegenStatus {
    ...
    pub fn gen_dupn(&mut self) -> CodegenStatus {
    ...
    pub fn gen_swap(&mut self) -> CodegenStatus {
        self.stack_swap(0, 1);
        CodegenStatus::KeepCompiling
    }
    ...
    pub fn gen_putnil(&mut self) -> CodegenStatus {
        self.jit_putobject(Qnil);
        CodegenStatus::KeepCompiling
    }

    pub fn jit_putobject(&mut self, arg: VALUE) {
        let val_type: Type = Type::from(arg);
        let stack_top = self.ctx.stack_push(val_type);
        self.asm.mov(stack_top, arg.into());
    }

    pub fn gen_putobject_int2fix(&mut self) -> CodegenStatus {
    ...
    pub fn gen_putobject(&mut self) -> CodegenStatus {
    ...
    pub fn gen_putself(&mut self) -> CodegenStatus {
    ...
    pub fn gen_putspecialobject(&mut self) -> CodegenStatus {
    ...
    pub fn gen_setn(&mut self) -> CodegenStatus {
        let n = self.jit.get_arg(0).as_usize();
    ...
    pub fn gen_topn(&mut self) -> CodegenStatus {
        let n = self.jit.get_arg(0).as_usize();
    ...
    pub fn gen_adjuststack(&mut self) -> CodegenStatus {
        let n = self.jit.get_arg(0).as_usize();
        self.ctx.stack_pop(n);
        CodegenStatus::KeepCompiling
    }

    pub fn gen_opt_plus(&mut self) -> CodegenStatus {
    ...
    pub fn gen_newarray(&mut self) -> CodegenStatus {
    ...
    // dup array
    pub fn gen_duparray(&mut self) -> CodegenStatus {
    ...
    // dup hash
    pub fn gen_duphash(&mut self) -> CodegenStatus {
    ...
    pub fn gen_splatarray(&mut self) -> CodegenStatus {
    ...
    // concat two arrays
    pub fn gen_concatarray(&mut self) -> CodegenStatus {
        // Save the PC and SP because the callee may allocate
        // Note that this modifies REG_SP, which is why we do it first
        self.jit_prepare_routine_call();
    ...
    pub fn gen_newrange(&mut self) -> CodegenStatus {
        let flag = self.jit.get_arg(0).as_usize();
    ...
    pub fn guard_object_is_string(&mut self, object_reg: Opnd, side_exit: Target) {
    ...
    pub fn gen_expandarray(&mut self) -> CodegenStatus {
    ...
    pub fn gen_get_ep(&mut self, level: u32) -> Opnd {
    ...
    pub fn gen_get_lep(&mut self) -> Opnd {
    ...
    pub fn gen_getlocal_generic(&mut self, ep_offset: u32, level: u32) -> CodegenStatus {
    ...
    pub fn gen_getlocal(&mut self) -> CodegenStatus {
        let idx = self.jit.get_arg(0).as_u32();
        let level = self.jit.get_arg(1).as_u32();
        self.gen_getlocal_generic(idx, level)
    }

    pub fn gen_getlocal_wc0(&mut self) -> CodegenStatus {
        let idx = self.jit.get_arg(0).as_u32();
        self.gen_getlocal_generic(idx, 0)
    }

    pub fn gen_getlocal_wc1(&mut self) -> CodegenStatus {
        let idx = self.jit.get_arg(0).as_u32();
        self.gen_getlocal_generic(idx, 1)
    }

    pub fn gen_setlocal_generic(&mut self, ep_offset: u32, level: u32) -> CodegenStatus {
    ...
    pub fn gen_setlocal(&mut self) -> CodegenStatus {
        let idx = self.jit.get_arg(0).as_u32();
        let level = self.jit.get_arg(1).as_u32();
        self.gen_setlocal_generic(idx, level)
    }

    pub fn gen_setlocal_wc0(&mut self) -> CodegenStatus {
        let idx = self.jit.get_arg(0).as_u32();
        self.gen_setlocal_generic(idx, 0)
    }

    pub fn gen_setlocal_wc1(&mut self) -> CodegenStatus {
        let idx = self.jit.get_arg(0).as_u32();
        self.gen_setlocal_generic(idx, 1)
    }

    // new hash initialized from top N values
    pub fn gen_newhash(&mut self) -> CodegenStatus {
    ...
    pub fn gen_putstring(&mut self) -> CodegenStatus {
    ...
    // the caller
    pub fn gen_checkkeyword(&mut self) -> CodegenStatus {
    ...
    pub fn gen_set_ivar(&mut self, ivar_name: ID, call_info: CallInfo, argc: i32) -> CodegenStatus {
    ...
    pub fn gen_getinstancevariable(&mut self) -> CodegenStatus {
    ...
    pub fn gen_setinstancevariable(&mut self) -> CodegenStatus {
        let starting_context = self.ctx.clone(); // make a copy for use with jit_chain_guard
    ...
    pub fn gen_defined(&mut self) -> CodegenStatus {
    ...
    pub fn gen_checktype(&mut self) -> CodegenStatus {
    ...
    pub fn gen_concatstrings(&mut self) -> CodegenStatus {
        let n = self.jit.get_arg(0).as_usize();
    ...
    pub fn guard_two_fixnums(&mut self, side_exit: Target) {
    ...
    pub fn gen_fixnum_cmp(&mut self, cmov_op: CmovFn, bop: ruby_basic_operators) -> CodegenStatus {
        let two_fixnums = match self.ctx.two_fixnums_on_stack(&mut self.jit) {
            Some(two_fixnums) => two_fixnums,
            None => {
    ...
    pub fn gen_opt_lt(&mut self) -> CodegenStatus {
        self.gen_fixnum_cmp(Assembler::csel_l, BOP_LT)
    }

    pub fn gen_opt_le(&mut self) -> CodegenStatus {
        self.gen_fixnum_cmp(Assembler::csel_le, BOP_LE)
    }

    pub fn gen_opt_ge(&mut self) -> CodegenStatus {
        self.gen_fixnum_cmp(Assembler::csel_ge, BOP_GE)
    }

    pub fn gen_opt_gt(&mut self) -> CodegenStatus {
    ...
    pub fn gen_equality_specialized(&mut self, gen_eq: bool) -> Option<bool> {
        // Create a side-exit to fall back to the interpreter
        let side_exit = self.get_side_exit(&self.ctx.clone());
    ...
    pub fn gen_opt_eq(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_neq(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_aref(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_aset(&mut self) -> CodegenStatus {
        // Defer compilation so we can specialize on a runtime `self`
        if !self.jit.at_current_insn() {
            self.defer_compilation();
            return CodegenStatus::EndBlock;
    ...
    pub fn gen_opt_and(&mut self) -> CodegenStatus {
        let two_fixnums = match self.ctx.two_fixnums_on_stack(&mut self.jit) {
            Some(two_fixnums) => two_fixnums,
            None => {
                // Defer compilation so we can specialize on a runtime `self`
    ...
    pub fn gen_opt_or(&mut self) -> CodegenStatus {
        let two_fixnums = match self.ctx.two_fixnums_on_stack(&mut self.jit) {
            Some(two_fixnums) => two_fixnums,
            None => {
                // Defer compilation so we can specialize on a runtime `self`
    ...
    pub fn gen_opt_minus(&mut self) -> CodegenStatus {
        let two_fixnums = match self.ctx.two_fixnums_on_stack(&mut self.jit) {
            Some(two_fixnums) => two_fixnums,
            None => {
                // Defer compilation so we can specialize on a runtime `self`
    ...
    pub fn gen_opt_mult(&mut self) -> CodegenStatus {
        // Delegate to send, call the method on the recv
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_div(&mut self) -> CodegenStatus {
        // Delegate to send, call the method on the recv
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_mod(&mut self) -> CodegenStatus {
        let two_fixnums = match self.ctx.two_fixnums_on_stack(&mut self.jit) {
            Some(two_fixnums) => two_fixnums,
            None => {
                // Defer compilation so we can specialize on a runtime `self`
    ...
    pub fn gen_opt_ltlt(&mut self) -> CodegenStatus {
        // Delegate to send, call the method on the recv
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_nil_p(&mut self) -> CodegenStatus {
        // Delegate to send, call the method on the recv
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_empty_p(&mut self) -> CodegenStatus {
        // Delegate to send, call the method on the recv
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_succ(&mut self) -> CodegenStatus {
        // Delegate to send, call the method on the recv
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_str_freeze(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_str_uminus(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_newarray_max(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_newarray_min(&mut self) -> CodegenStatus {
        let num = self.jit.get_arg(0).as_u32();
    ...
    pub fn gen_opt_not(&mut self) -> CodegenStatus {
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_size(&mut self) -> CodegenStatus {
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_length(&mut self) -> CodegenStatus {
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_regexpmatch2(&mut self) -> CodegenStatus {
        self.gen_opt_send_without_block()
    }

    pub fn gen_opt_case_dispatch(&mut self) -> CodegenStatus {
    ...
    pub fn gen_branchif(&mut self) -> CodegenStatus {
        let jump_offset = self.jit.get_arg(0).as_i32();

        // Check for interrupts, but only on backward branches that may create loops
        if jump_offset < 0 {
            let side_exit = self.get_side_exit(&self.ctx.clone());
            self.gen_check_ints(side_exit);
        }
    ...
    pub fn gen_branchunless(&mut self) -> CodegenStatus {
        let jump_offset = self.jit.get_arg(0).as_i32();
    ...
    pub fn gen_branchnil(&mut self) -> CodegenStatus {
        let jump_offset = self.jit.get_arg(0).as_i32();
    ...
    pub fn gen_jump(&mut self) -> CodegenStatus {
        let jump_offset = self.jit.get_arg(0).as_i32();
    ...
    pub fn gen_opt_send_without_block(&mut self) -> CodegenStatus {
        let cd = self.jit.get_arg(0).as_ptr();
        self.gen_send_general(cd, None)
    }

    /// self.jit_save_pc() + self.gen_save_sp(). Should be used before calling a routine that
    /// could:
    ///  - Perform GC allocation
    ///  - Take the VM lock through RB_VM_LOCK_ENTER()
    ///  - Perform Ruby method call
    pub fn jit_prepare_routine_call(&mut self) {
    ...
    pub fn gen_send_cfunc(
        ...
        self.record_global_inval_patch(CodegenGlobals::get_outline_full_cfunc_return_pos());
    ...
    pub fn gen_send_iseq(
        ...
        // that could look at it through self.jit_save_pc().
    ...
    pub fn gen_send(&mut self) -> CodegenStatus {
        let cd = self.jit.get_arg(0).as_ptr();
        let block = self.jit.get_arg(1).as_optional_ptr();
        self.gen_send_general(cd, block)
    }

    pub fn gen_invokeblock(&mut self) -> CodegenStatus {
        if !self.jit.at_current_insn() {
            self.defer_compilation();
            return CodegenStatus::EndBlock;
        }
    ...
    pub fn gen_invokesuper(&mut self) -> CodegenStatus {
        let cd: *const rb_call_data = self.jit.get_arg(0).as_ptr();
    ...
    pub fn gen_leave(&mut self) -> CodegenStatus {
    ...
    pub fn gen_getglobal(&mut self) -> CodegenStatus {
    ...
    pub fn gen_setglobal(&mut self) -> CodegenStatus {
        let gid = self.jit.get_arg(0).as_usize();
    ...
    pub fn gen_anytostring(&mut self) -> CodegenStatus {
    ...
    pub fn gen_objtostring(&mut self) -> CodegenStatus {
        if !self.jit.at_current_insn() {
            self.defer_compilation();
            return CodegenStatus::EndBlock;
        }
    ...
    pub fn gen_intern(&mut self) -> CodegenStatus {
    ...
    pub fn gen_toregexp(&mut self) -> CodegenStatus {
    ...
    pub fn gen_getspecial(&mut self) -> CodegenStatus {
    ...
    pub fn gen_getclassvariable(&mut self) -> CodegenStatus {
    ...
    pub fn gen_setclassvariable(&mut self) -> CodegenStatus {
    ...
    pub fn gen_getconstant(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_getconstant_path(&mut self) -> CodegenStatus {
    ...
    pub fn gen_getblockparamproxy(&mut self) -> CodegenStatus {
        if !self.jit.at_current_insn() {
            self.defer_compilation();
            return CodegenStatus::EndBlock;
        }
    ...
    pub fn gen_getblockparam(&mut self) -> CodegenStatus {
        // EP level
        let level = self.jit.get_arg(1).as_u32();
    ...
    pub fn gen_invokebuiltin(&mut self) -> CodegenStatus {
    ...
    pub fn gen_opt_invokebuiltin_delegate(&mut self) -> CodegenStatus {
        let bf: *const rb_builtin_function = self.jit.get_arg(0).as_ptr();
    ...
    pub fn get_side_exit(&mut self, ctx: &Context) -> Target {
        match self.jit.side_exit_for_pc {
            None => {
                let exit_code = self.gen_outlined_exit(self.jit.pc, ctx);
